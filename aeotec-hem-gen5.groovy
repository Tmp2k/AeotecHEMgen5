...
metadata {
	...
	tiles (scale: 2) {
		multiAttributeTile(name:"multiTile", type: "generic", width: 3, height: 4){
			tileAttribute ("device.power", key: "PRIMARY_CONTROL") {
				attributeState "power", label: '${currentValue} kW', icon: null, backgroundColors:[
					...
					[value: 6, color: "#d04e00"],
					[value: 9, color: "#bc2323"]
				]
			}
        ...
        valueTile("meterReading", "device.meterReading", decoration: "flat", width: 2, height: 2) {
			state "meterReading", label:'${currentValue}'
		}
        
        
		standardTile("reset", "device.reset", decoration: "flat", width: 2, height: 2) {
			state "default", label:'Reset kWh', action:"reset", icon: "http://cdn.device-icons.smartthings.com/Kids/kids4-icn@2x.png"
		}
		...
		main "totalCost"
		details("multiTile","energy","voltage","current","totalCost","meterReading","reset","refresh")
	}
	...
	preferences {
        ...
        input ( name: "dailyCost", title: "Daily standing charge", type: "decimal", required: false )
        input ( name: "unitCost", title: "Cost per kWh", type: "decimal", required: false )
        input ( name: "meterOffset", title: "Existing meter offset", type: "number", required: false )
...
def resetMeter() {
	//This just updates the resetOffset it does NOT change the value on the meter
    
    logging("${device.displayName} - executing resetMeter()","info")
    
    //reset master
    sendEvent([name: "resetOffset", value: device.currentValue("originalEnergy"), displayed: false])
    (zwaveInfo.epc as Integer)?.times { n ->
    	//reset child
        getChild(n+1)?.sendEvent([name: "resetOffset", value: getChild(n+1)?.currentValue("originalEnergy"), displayed: false])
	}
...
def hardResetMeter() {
	//This resets the meter to 0 (it's not currently used anywhere)
    
	logging("${device.displayName} - executing hardResetMeter()","info")
	def cmds = []
...
def allChildrenRefresh() {
	logging("${device.displayName} - executing allChildrenRefresh()...","info")
	(zwaveInfo.epc as Integer)?.times { n ->
    	childRefresh("${device.deviceNetworkId}-c${n+1}")
	}
}
...
def zwaveEvent(physicalgraph.zwave.commands.meterv3.MeterReport cmd, ep=null) {
	...
	switch (cmd.scale+((cmd.scale2) ? 1:0)) {
		case 0: type = "energy"; 			unit = "kWh"; 	value = Math.round(cmd.scaledMeterValue); break; 
		case 1: type = "totalEnergy"; 		unit = "kVAh"; 	value = Math.round(cmd.scaledMeterValue); break;
		case 2: type = "power"; 			unit = "kW"; 	value = Math.round(cmd.scaledMeterValue/100)/10; break;	
		case 4: type = "voltage"; 			unit = "V"; 	value = Math.round(cmd.scaledMeterValue); break;
		case 5: type = "current"; 			unit = "A"; 	value = Math.round(cmd.scaledMeterValue); break;
		case 7: type = "reactivePower"; 	unit = "kVar"; 	value = Math.round(cmd.scaledMeterValue); break;
		case 8: type = "reactiveEnergy"; 	unit = "kVarh"; value = Math.round(cmd.scaledMeterValue); break; 
	}
    
    
    
    
	logging("${device.displayName} - MeterReport received, ep: ${((ep) ? ep:0)} value: ${value} ${unit}", "info")
	if (ep == null) {
    
    	//calculate costs
        if(type == "energy") {
            sendEvent([name: "originalEnergy", value: value, unit: unit, displayed: false])
            sendEvent([name: "meterReading", value: value+settings.meterOffset, unit: unit, displayed: false])
        	value = value - device.currentValue("resetOffset");
        	sendEvent([name: "totalCost", value: "£ ${Math.round(value*settings.unitCost)/100}", displayed: false])
    	}
        
		sendEvent([name: type, value: value, unit: unit, displayed: false])
    	...
    	//calculate costs
        if(type == "energy") {
        	getChild(ep)?.sendEvent([name: "originalEnergy", value: value, unit: unit, displayed: false])
        	value = value - getChild(ep)?.currentValue("resetOffset");
        	getChild(ep)?.sendEvent([name: "totalCost", value: "£ ${Math.round(value*settings.unitCost)/100}", displayed: false])
        }
    	
		getChild(ep)?.sendEvent([name: type, value: value, unit: unit, displayed: false]) 
